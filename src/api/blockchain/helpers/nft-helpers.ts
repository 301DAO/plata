...
import { Stats } from '@/api/blockchain/types/opensea.types'
import { openseaCollectionStats, openseaAddressCollections } from '@/api/blockchain/opensea'
...
export const getAddressNFTData = async (address: string): Promise<NftData | null> => {
  try {
    ...
    const formatted = filtered.map((item, index) => {
      ...
      return { collection: collections[index], floor, monthlyChange, floor_thirty_days_ago, change }
    })
    if (!formatted.length) {
      return null
    }
